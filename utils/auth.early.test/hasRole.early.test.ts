...
describe("hasRole() hasRole method", () => {
  ...
  describe("Edge Cases", () => {
    ...
    it("should return true when both roles are the lowest in hierarchy", () => {
      // Test that the function returns true when both userRole and requiredRole are the lowest in the hierarchy
      const userRole: MockRole = "Non-Member" as any;
      const requiredRole: MockRole = "Non-Member" as any;
      expect(hasRole(userRole, requiredRole)).toBe(true);
    });
  });
});
